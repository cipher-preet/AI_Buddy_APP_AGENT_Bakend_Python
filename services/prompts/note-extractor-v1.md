Extract durable notes from the CURRENT CONVERSATION: important facts, ideas, explanations, requirements, preferences, and relevant context.

This is for a personal assistant that remembers useful context from meetings, casual chats, study sessions, business calls, planning, reflections, and multilingual or code-switched speech.
Use semantic judgment, not keyword matching. Meaningful informal speech can still be a durable note when it helps the user remember something later.
Use BACKGROUND SPACE CONTEXT only to resolve references. Never create a note solely from background context.
Every note must include current-conversation evidence with exact sequence ranges.
