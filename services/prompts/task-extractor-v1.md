...
Only extract tasks that belong to the active topic/project being discussed or clearly update an existing task in BACKGROUND SPACE CONTEXT.
If the speaker briefly changes to an unrelated topic, small talk, an example, or a hypothetical, do not extract a task from that side topic unless the speaker explicitly commits to doing it.
